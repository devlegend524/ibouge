...
module.exports = {
    ...
    deleteStatus: function(id, from) {
        return new Promise((resolve, reject) => {
            ...
            statusUpdate.findOneAndUpdate({"status._id": id},
            {
                $pull: {
                    status: {
                        _id: id
    ...
    deleteReply: function(reply_id, status_id) {
        return new Promise((resolve, reject) => {
            // update statusUpdate Schema for user
            statusUpdate.update({'status._id' : status_id},
            {
                $pull: {
                    'status.$.replies': {
                        _id: reply_id
    ...
    createNewStatus: function (from, statusType, message, time, size, filename, caption, likes) {
        return new Promise((resolve, reject) => {
            ...
            statusUpdate.findOne({'user': from}, function(err, status) {
                ...
                // update statusUpdate Schema for user
                statusUpdate.findOneAndUpdate({user: from}, {
                    $push: {
                        status: {
                            $each: [{
                                status_type: statusType,
                    ...
                    select: {status:{$slice: 1}}}, function(err, status) {
                    if (err) {
                        return reject({status: 500, message: 'Internal server error'});
                    }
    ...
    createNewReply: function (status, from, replyType, message, time, cb) {
        ...
        // update statusUpdate Schema for user
        statusUpdate.findOneAndUpdate({'status._id': status}, {
            $push: {
                'status.$.replies': {
                    $each: [{
                ...
                select: {status:{$slice: 1}}}, function(err, reply) {
            if (err) {
                        ...
                        // if photo saves successfully
                            ...
                            statusUpdate.findOne({'user': data.userId}, function(err, status) {
                                ...
                                if (!status) {
                                    ...
                                    // update statusUpdate Schema for user
                                    statusUpdate.findOneAndUpdate({user: data.userId}, {
                                        $push: {
                                            status: {
                                                $each: [{
                                                    status_type: data.status_type,
    ...
    addLikeToStatusOrReply: function (id, like, cb) {
        ...
        if (like.reply) {
            ...
            statusUpdate.findOne(find, {"status.$.replies": 1}, function(err, statusData) {
                if (err) {
                ...
                if (index != -1) {
                    ...
                    update['$push'][str] = { $each: [{from: like.from, date: like.when}], $position: 0};
            ...
            update = {
                $push: { "status.$.likes": { $each: [{from: like.from, date: like.when}], $position: 0}}
            };
