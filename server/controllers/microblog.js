...
module.exports = {
    ...
    getMicroblogHistory: function(room, limit, offset) {
        ...
        Microblog.findOne({'room': room}, {messages: {$slice: [offset, limit]}},
    ...
    _getMicroblogMeta: function(microblog) {
        return new Promise((resolve, reject) => {
            ...
            User.find({'_id': {$in: userIds}}, ['fname', 'lname', 'profile_pic'],
        ...
        return new Promise((resolve, reject) => {
            ...
            if (data.file) {
                ...
                if (generatedString !== '') {
                    ...
                    var photoKey = albumPhotosKey + data.userId + '_' + data.room + '_' + generatedString;
                        ...
                        s3bucket.upload(params, function(err, photo) {
                            ...
                            // else, if photo saves successfully
                                ...
                                Microblog.findOne({'room': data.room}, function(err, microblog) {
                                    ...
                                    User.find({'_id': {$in: data.users}}, ['fname', 'lname', 'profile_pic'],function(err, _microblogUsers) {
                                        ...
                                        if (!_microblogUsers || _microblogUsers.length === 0) {
                                            return reject({status: 404, message: 'No users found for microblog'});
                                        }

                                        var microblogUsers = _microblogUsers.map(function(user) {
                                            return data.userId === user._id ? {user_id : user._id, last_login: Date.now()} : {user_id : user._id};
                                        });
                                        ...
                                        newMicroblog.save(function(err) {
                                                ...
                                                function(err, user) {
                                                    ...
                                                    for (var i = 0; i < microblogs.length; i++) {
                                                        if (data.room === microblogs[i]) {
                                                            return reject({status: 401, message: 'microblog has already been added'});
                                                        }
                                                    }
                                            ...
                                            return resolve({microblog: newMicroblog, users: _microblogUsers});
                                        });
                                    });
                                });
                            }
                                ...
                                function(err, user) {
                                    ...
                                    user.save(function(err) {
                                        if (err) {
                                            ...
                                            console.log("error happened while saving new microblog room to user's bookmarked_microblogs: ", err);
    ...
    updateMicroblogPic: function(room, data) {
        return new Promise((resolve, reject) => {
            ...
            function(err, microblog) {
                ...
                fileHandler.createFile(name, img, 'upload').then(function() {
                    ...
                    fs.readFile(data.file.path, function (err, fileData) {
                        ...
                        s3bucket.upload(params, function (err, photo) {
                            ...
                            // if photo saves successfully
                                ...
                                Microblog.findOneAndUpdate({room: data.microblogRoom}, {
                                    $set: {
            ...
            Microblog.findOne({'room': room},
                ...
                for (var i = 0; i < users.length; i++) {
                    if (user === users[i].user_id) {
                        return reject({status: 401, message: 'user has already been added'});
                    }
                }
    ...
    addMeToAllInvolved: function(data) {
        return new Promise((resolve, reject) => {
            Microblog.findOne({'room': data.room},
            function(err, microblog) {
                if (err) {
                    return reject({status: 500, message: 'Internal server error'});
                }
                ...
                Microblog.update({room: data.room}, {
                    $push: {
    ...
    setLastLogoutMicroblog: function(userId, room) {
        Microblog.update({'room': room, 'users.user_id': userId}, {
    ...
    getMicroblogsByCreatedBy: function(userId, fields) {
        return new Promise((resolve, reject) => {
            ...
            Microblog.find({'created_by': userId, 'is_microblog': true}, fields,
