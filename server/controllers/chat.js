...
module.exports = {
	...
	getHistory: function(room, limit, offset) {
		return new Promise((resolve, reject) => {
			...
			Chat.findOne({'room': room}, {messages: {$slice: [offset, limit]}}, function(err, chat) {
				if (err) {
	...
	updateLastMessage: function(room) {
		return new Promise((resolve, reject) => {
			...
			Chat.findOneAndUpdate({'room': room}, {'$set': {"messages.0.read_at": Date.now()}}, function(err, chat) {
				if (err) {
					return reject({status: 500, message: 'Internal server error'});
				}
	...
	_getChatMeta: function(chat) {
		return new Promise((resolve, reject) => {
			...
			User.find({'_id': {$in: userIds}}, ['fname', 'lname', 'profile_pic'], function(err, chatUsers) {
				if (err) {
					return reject({status: 500, message: 'Internal server error'});
				}
	...
	createChat: function(myId, room, users, isGroupChat, groupChatName) {
		return new Promise((resolve, reject) => {
			...
			Chat.findOne({'room': room}, function(err, chat) {
				...
				User.find({'_id': {$in: users}}, ['fname', 'lname', 'profile_pic'], function(err, _chatUsers) {
					if (err) {
						return reject({status: 500, message: 'Internal server error'});
					}
					...
					var chatUsers = _chatUsers.map(function(user) {
						return myId == user._id ? {user_id : user._id, last_login: Date.now()} : {user_id : user._id};
					});
			...
			if (data.file) {
				...
				if (generatedString !== '') {
					...
					fs.readFile(data.file.path, function(err, fileData) {
						...
						var params = {Bucket: 'ibouge', Key: photoKey, Body: fileData,
						ACL: 'public-read'};
						...
						s3bucket.upload(params, function (err, photo) {
							...
							// if photo saves successfully
								...
								Chat.findOneAndUpdate({room: data.chatRoom}, {
									$set: {
                    last_updated_date: Date.now()
									},
									$push: {
	...
	setLastLogout: function(userId, room) {
		Chat.update({'room': room, 'users.user_id': userId}, {
	...
	getGroupChatsByCreatedBy: function(userId, fields) {
		return new Promise((resolve, reject) => {
			...
			Chat.find({'created_by': userId, 'is_group_chat': true}, fields, function(err, chats) {
				if (err || !chats) {
					return reject({status: 500, message: 'Internal server error'});
				}
